...
def update(olist, plist):
    """
    Allows the user to update their current order.
    ...
    while run is True:
        ...
        if option == "C":
            ...
            item = validate_index("What pizza would you like to update? (please enter the correlating item number)", 0,
                                  len(olist) - 1,
                                  "This is not a valid item number, it is too low. Please enter a larger item number",
                                  "This entry is too big and/or too many digits, please try again and enter a smaller item number.")
            print("Currently, you have {} {} pizza/s in your order".format(olist[item][1],olist[item][0]))
            update_quantity = validate_quantity("How many {} pizzas would you like to order?: ".format(olist[item][0]), 0, 30, "Sorry you must have a final order at least one pizza, please order a quantity bigger than 0")
            print("Your order has been updated, you have now order {} {} pizzas".format(update_quantity, olist[item][0]))
            ...
            delete_item = validate_index("Which pizza order would you like to delete? (please enter the correlating item number)", 0,
                                         len(olist) - 1,
                                         "This is not a valid index number, it is too low. Please enter a larger index number",
                                         "This entry is too big and/or too many digits, please try again and enter a smaller index number."
                                         )
            print("Currently, you have {} {} pizza/s in your order".format(olist[delete_item][1], olist[delete_item][0]))
